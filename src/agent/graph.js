...
try {
    ...
    console.warn('Rebalancer not configured:', error.message);
}
// Create tools
const tools = createWardenTools(agentkit, priceFetcher, swapExecutor, rebalancer);
// Initialize LLM
 ...
 */
async function agentNode(state, _config) {
 ...
 */
async function _checkTriggersNode(state) {
    ...
    for (const trigger of triggers) {
        ...
        try {
            ...
            console.error(`Error checking trigger ${trigger.id}:`, error.message);
            updatedTriggers.push(trigger);
        }
    }
    return {
 ...
 */
async function _rebalanceNode(_state) {
    if (!rebalancer) {
        return {
    ...
    try {
        ...
        return {
            ...
            needsRebalancing: false,
            messages: [
                new AIMessage(`✅ Portfolio rebalanced successfully! Completed ${status.rebalanceCount} rebalances total.`),
            ],
        };
    }
    catch (error) {
        console.error('Error rebalancing:', error.message);
        return {
            messages: [
                new AIMessage(`❌ Rebalancing failed: ${error.message}`),
            ],
            needsRebalancing: false,
        };
    }
 ...
 */
async function updatePortfolioNode(state) {
    ...
    try {
        ...
        let maxDrift = 0;
        for (const token of tokens) {
            const drift = Math.abs(allocation[token] - (targetAllocation[token] || 0));
            if (drift > maxDrift) {
                maxDrift = drift;
            }
        }
 ...
 */
function _shouldCheckTriggers(state) {
 ...
 */
function _shouldRebalance(state) {
 ...
 */
export async function invokeAgent(message, walletAddress, threadId = 'default') {
    const config = {
        configurable: {
            thread_id: threadId,
        },
 ...
 */
export async function streamAgent(message, walletAddress, threadId = 'default') {
    const config = {
        configurable: {
            thread_id: threadId,
        },

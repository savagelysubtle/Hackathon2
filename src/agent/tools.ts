...
export function createWardenTools(
  ...
  return [
    ...
    new DynamicStructuredTool({
      name: 'get_portfolio',
      description:
        'Get current portfolio allocation and balances for the wallet. ' +
      ...
      func: async ({ walletAddress }) => {
        try {
          ...
          return JSON.stringify({
            error: `Failed to get portfolio: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      name: 'create_trigger',
      description:
        'Create a price-based trigger for automatic execution. ' +
      ...
      func: async ({ asset, condition, threshold, action }) => {
        try {
          ...
          const trigger = {
            ...
            condition,
            threshold:
              condition === 'dump' ? -Math.abs(threshold) : Math.abs(threshold),
            action,
            active: true,
            progress: 0,
          ...
          return JSON.stringify({
            error: `Failed to create trigger: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      name: 'check_triggers',
      description:
        'Check all active triggers and return their current status. ' +
      ...
      func: async ({ triggers }) => {
        try {
          ...
          for (const trigger of triggers) {
            ...
            const currentPrice = await priceFetcher.getPrice(
              `${trigger.asset}/USD`,
            );
            const baselinePrice = trigger.baselinePrice || currentPrice;

            const change = priceFetcher.calculateChange(
              currentPrice,
              baselinePrice,
            );
            const progress =
              (Math.abs(change) / Math.abs(trigger.threshold)) * 100;
          ...
          return JSON.stringify({
            error: `Failed to check triggers: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      description:
        'Check if portfolio rebalancing is needed based on current drift from target allocations.',
      schema: z.object({
        portfolio: z
          .object({
      ...
      func: async ({ portfolio }) => {
        try {
          ...
          return JSON.stringify({
            error: `Failed to check rebalancing: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      description:
        'Execute portfolio rebalancing to bring allocations back to target percentages.',
      schema: z.object({
      ...
      func: async ({ execute = true }) => {
        try {
          ...
          return JSON.stringify({
            error: `Failed to rebalance portfolio: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async ({ asset }) => {
        try {
          ...
          return JSON.stringify({
            error: `Failed to get price: ${(error as Error).message}`,
          });
        }
      },
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async ({ assets }) => {
        try {
          ...
          return JSON.stringify({
            error: `Failed to get prices: ${(error as Error).message}`,
          });
        }
      },
    }),

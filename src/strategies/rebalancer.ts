...
export interface RebalanceConfig {
  ...
  driftThreshold: number; // e.g., 5 for 5% drift tolerance
...
export class PortfolioRebalancer {
  ...
  constructor(
    ...
    const totalPercent = config.targets.reduce(
      (sum, t) => sum + t.targetPercent,
      0,
    );
    if (Math.abs(totalPercent - 100) > 0.01) {
      throw new Error(
        `Target allocations must sum to 100% (got ${totalPercent}%)`,
      );
    }
  }
  ...
  async rebalance(): Promise<void> {
    ...
    try {
      ...
      console.error('\n❌ Rebalance failed:', (error as Error).message);
      throw error;
    }
  }
   ...
   */
  private displaySnapshot(snapshot: PortfolioSnapshot): void {
    ...
    for (const [asset, amount] of Object.entries(snapshot.holdings)) {
      ...
      console.log(
        `      ${asset}: ${amount.toFixed(6)} @ $${price.toFixed(2)} = $${value.toFixed(2)}`,
      );
    }
   ...
   */
  private displayAllocations(snapshot: PortfolioSnapshot): void {
    for (const target of this.config.targets) {
      const current = snapshot.allocations[target.asset];
      const drift = current - target.targetPercent;
      const driftSign = drift >= 0 ? '+' : '';
      const status =
        Math.abs(drift) <= this.config.driftThreshold ? '✅' : '⚠️ ';

      console.log(
        `      ${status} ${target.asset}: ${current.toFixed(2)}% (target: ${target.targetPercent}%, drift: ${driftSign}${drift.toFixed(2)}%)`,
      );
    }
  }
   ...
   */
  private calculateRebalanceTrades(snapshot: PortfolioSnapshot): any[] {
    const trades: any[] = [];
  ...
  private async executeTrade(
    ...
    console.log(
      `      ${trade.action}: $${Math.abs(trade.deltaValue).toFixed(2)}`,
    );
   ...
   */
  async needsRebalance(): Promise<boolean> {
    const snapshot = await this.getPortfolioSnapshot();
   ...
   */
  getHistory(limit: number = 10): PortfolioSnapshot[] {

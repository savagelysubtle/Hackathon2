...
export class PriceFetcher {
  ...
  async getPrice(currencyPair, useCache = true) {
    ...
    try {
      ...
      console.error(
        `❌ Failed to fetch price for ${currencyPair}:`,
        error.message,
      );
      throw error;
    }
  ...
  async waitForPriceThreshold(
    ...
    console.log(
      `⏳ Monitoring ${currencyPair} until it reaches $${threshold}...`,
    );
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (currentPrice >= threshold) {
        console.log(
          `✅ Price threshold reached: $${currentPrice} >= $${threshold}`,
        );
        return currentPrice;
      }
      console.log(
        `   Current: $${currentPrice}, Target: $${threshold} (waiting...)`,
      );
      await new Promise((resolve) => setTimeout(resolve, checkInterval));
    }
    throw new Error(
      `Timeout: Price did not reach $${threshold} within ${timeout}ms`,
    );
  }
  /**
   * Clear price cache

...
describe('LangGraph Agent', () => {
  ...
  describe('Portfolio Tools', () => {
    ...
    it('should check if rebalancing is needed', async () => {
      const result = await invokeAgent(
        'Check if my portfolio needs rebalancing',
        testWalletAddress,
  ...
  describe('Trigger Tools', () => {
    ...
    it('should check active triggers', async () => {
      ...
      await invokeAgent(
        'Create a trigger to sell 5% ETH if it dumps 15%',
        testWalletAddress,
        threadId,
      );
      // Then check triggers
      const result = await invokeAgent(
        'Check all my active triggers',
        testWalletAddress,
        threadId,
      );
      expect(result.messages).toBeDefined();
  ...
  describe('Price Tools', () => {
    ...
    it('should get multiple prices', async () => {
      const result = await invokeAgent(
        'Show me the current prices for ETH, SOL, and BTC',
        testWalletAddress,
        'test-multi-price',
      );
      expect(result.messages).toBeDefined();
  ...
  describe('Streaming', () => {
    it('should stream agent responses', async () => {
      const stream = await streamAgent(
        'Explain how portfolio rebalancing works',
        testWalletAddress,
        'test-streaming',
      );
      let chunkCount = 0;
  ...
  describe('Error Handling', () => {
    it('should handle invalid tool calls gracefully', async () => {
      const result = await invokeAgent(
        'Execute a swap from INVALID_TOKEN to ETH',
        testWalletAddress,
        'test-error-handling',
      );
      expect(result.messages).toBeDefined();
  ...
  describe('Integration Tests', () => {
    it('should handle complex multi-step workflow', async () => {
      ...
      const result1 = await invokeAgent(
        'Show me my portfolio',
        testWalletAddress,
        threadId,
      );
      ...
      const result2 = await invokeAgent(
        'Create a trigger to sell 10% if any asset pumps 25%',
        testWalletAddress,
        threadId,
      );
      expect(result2.triggers).toBeDefined();
      // Step 3: Check rebalancing
      const result3 = await invokeAgent(

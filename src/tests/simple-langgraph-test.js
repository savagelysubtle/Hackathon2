...
async function runSimpleTests() {
    ...
    try {
        ...
        const result = await invokeAgent('Hello! Can you explain what you can help me with?', testWalletAddress, 'test-simple');
    ...
    try {
        ...
        const result = await invokeAgent('What is the current price of ETH?', testWalletAddress, 'test-tools');
        if (result.messages && result.messages.length > 0) {
    ...
    try {
        ...
        const result = await invokeAgent('Show me my portfolio', testWalletAddress, 'test-portfolio');
        if (result.messages && result.messages.length > 0) {
...
//# sourceMappingURL=simple-langgraph-test.js.map
...
for (const test of tests) {
  ...
  console.log(
    `   $${test.baseline} → $${test.current}: ${result.toFixed(2)}% ${pass ? '✅' : '❌'}`,
  );
}
console.log();
...
console.log('─────────────────────────────────────────────────');
function shouldTrigger(currentPrice, config) {
...
for (const test of priceTests) {
  ...
  console.log(
    `   $${test.price} (${change.toFixed(1)}%): ${status} ${pass ? '✅' : '❌'}`,
  );
}
console.log();
...
console.log('─────────────────────────────────────────────────');
function calculateSellAmount(holdings, sellPercent) {
...
for (const test of holdingsTests) {
  ...
  const pass = Math.abs(result - test.expected) < 0.01;
  console.log(
    `   ${test.holdings} ${test.percent}% = ${result.toFixed(2)} ${pass ? '✅' : '❌'}`,
  );
}
console.log();
...
function checkTrigger(price) {
  ...
  console.log(
    `   Check ${state.checkCount}: $${price} (${change.toFixed(1)}%)`,
  );
  ...
  console.log(
    `      ⏳ Waiting (need ${(config.triggerPercent - change).toFixed(1)}% more)`,
  );
  return false;
}
...
console.log('═══════════════════════════════════════════════════\n');
//# sourceMappingURL=test-trigger-logic.js.map

...
async function testOracle() {
    ...
    try {
        ...
        if (triggerChange >= triggerPercent) {
            console.log(`   🚀 TRIGGER FIRED! SOL pumped ${triggerChange.toFixed(2)}%`);
            console.log(`   ✅ Would execute: Sell 10% of SOL holdings`);
        } else {
            console.log(`   ⏳ Trigger not fired (need ${triggerPercent}%, current: ${triggerChange.toFixed(2)}%)`);
        ...
        console.log('   2. ⏭️  Build PriceTrigger: bun run src/triggers/price-trigger.ts');

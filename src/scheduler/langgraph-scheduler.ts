...
export class LangGraphScheduler {
  ...
  scheduleRebalancing(schedule: string, walletAddress: string) {
    const job = cron.schedule(schedule, async () => {
      ...
      try {
        ...
        const result = await graph.invoke(
          {
            messages: [
              new HumanMessage(
                'Check if portfolio rebalancing is needed and execute if necessary',
              ),
            ],
            walletAddress,
          },
        ...
        console.error('❌ Rebalancing error:', (error as Error).message);
      }
    });
  ...
  scheduleTriggerChecks(schedule: string, walletAddress: string) {
    const job = cron.schedule(schedule, async () => {
      ...
      try {
        ...
        const result = await graph.invoke(
          {
            messages: [
              new HumanMessage(
                'Check all active price triggers and report their status',
              ),
            ],
            walletAddress,
          },
        ...
        console.error('❌ Trigger check error:', (error as Error).message);
      }
    });
   ...
   */
  schedulePortfolioUpdate(schedule: string, walletAddress: string) {
    const job = cron.schedule(schedule, async () => {
      ...
      try {
        ...
        console.error('❌ Portfolio update error:', (error as Error).message);
      }
    });

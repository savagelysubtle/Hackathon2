...
export class CronScheduler {
    ...
    private async executeJob(job: ScheduledJob): Promise<void> {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            console.error('\n─────────────────────────────────────────────────');
            console.error(`❌ Job failed after ${duration}ms:`, (error as Error).message);
            console.error('═══════════════════════════════════════════════════\n');
     ...
     * Log job execution
     */
    private logExecution(log: JobLog): void {
     ...
     * Get execution logs
     */
    getExecutionLogs(jobId?: string, limit: number = 10): JobLog[] {
